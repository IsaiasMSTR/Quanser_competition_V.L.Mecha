...
class ImageProcessing():
    ...
    def __init__(self):
        ...
        self.visualizeLines = [([],[])]
        #self.frame = np.array([820,410]),

        
        #elf.frame = np.ndarray()
        #self.line = np.array([],[],[],[])
        #self.calculateCoordinates =np.array([])
    ...
    def calculate_lines_version_isaias(self, segment):
        # +--------> x-axis
        # |               /          |               \
        # |             /            | Inf slope      \                     0 slope
        # v           / -ve slope    |                 \ +ve slope      _______________
        ...
        lines = cv2.HoughLinesP(segment, 1, np.pi/180, 10,1,1)
        #lines = cv2.HoughLinesP(segment, 2, np.pi/180, 100, np.array([]),200)
    ...
    def driving_parameters(self, lineParameters):
        ...
        if lineParameters[1] is not None:#is not None:
...
#----------------------------------READY---------------------------------------
    ...
    def detect_yelllow_lane(self, frame):
        lowerBounds = (15, 100, 20)
        upperBounds = (125, 255, 255)
        mask = cv2.inRange(frame, lowerBounds, upperBounds)
        mask = cv2.dilate(mask, np.ones((5, 5), np.uint8), iterations=1)
        return mask
    
    def detect_red_laneBAJO(self, frame):
        lowerBounds = (0,100,20)
        upperBounds = (5, 255, 255)
        mask = cv2.inRange(frame, lowerBounds, upperBounds)
        mask = cv2.dilate(mask, np.ones((5, 5), np.uint8), iterations=1)
        return mask
    
       
    def detect_red2_lane2ALTO(self, frame):
        lowerBounds = (175, 100, 230)
        upperBounds = (179, 255, 255)
        #lowerBounds = (175, 100, 230)
        #upperBounds = (179, 255, 255)
        mask = cv2.inRange(frame, lowerBounds, upperBounds)
        mask = cv2.dilate(mask, np.ones((5, 5), np.uint8), iterations=1)
...
#------------------------ready------------------------------------------------
    def binary_thresholding(self, frame, lowerBounds, upperBounds):
        ...
        if rs == 3: # 3-color (BGR, RGB, HSV) Image input
            binary = cv2.inRange(frame, lowerBounds, upperBounds)
        elif rs == 2: # Grayscale Image input
    ...
    def image_filtering_close(self,frame, dilate=1, erode=1, total=1):
    ...
    def image_filtering_open(self,frame, dilate=1, erode=1, total=1):
    ...
    def image_filtering_skeletonize(self,frame):
...
#--------------------------------------------READY------------------------------------------------------
    def mask_image(self,frame, rowUp, rowDown, colLeft, colRight):
    ...
    def extract_lane_points_by_row(self,frame, row):
    ...
    def find_slope_intercept_from_binary(self,binary):
...
#----------------------------------------END READY----------------------------------------------------
    def get_perspective_transform(self,ptsUpperRow, ptsLowerRow):
    ...
    def circle_pts(self,frame, pts, radius, color):
        ...
        for i in range(pts.shape[0]):
            cv2.circle(frame, (( int (pts[i][0])), (int (pts[i][1]))), radius, ( int(color[0]), int(color[1]), int(color[2]) ), thickness=3 )
